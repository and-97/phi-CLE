...
int main(){
    ...
    field f(global_settings.nodes, 'a', global_settings.t_max, global_settings.beta, global_settings.F1, global_settings.F2);
    ...
    for(traj = 0; traj < global_settings.n_trajectories; traj++){
        ...
        for(t = 0.; t < global_settings.simulation_Ltime;){
            ...
            if(t-last_ms_time > global_settings.ms_frequency){
                ...
                last_ms_time = t;
                for(int i = 0; i < global_settings.nodes; i++){
                    output_file << f.get_field(i).real() << " " << f.get_field(i).imag() << std::endl;
                }
            }
        }
    }
